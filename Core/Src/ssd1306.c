...
void ssd1306_UpdateScreenRightSide(uint8_t *inData) {
	for(uint8_t i = 0; i < SSD1306_HEIGHT/8; i++) {
		ssd1306_WriteCommand(0xB0 + i); // Set the current RAM page address.
		ssd1306_WriteCommand(0x0F);		// Low 4bit
		ssd1306_WriteCommand(0x17);		// High 4bit
		ssd1306_WriteData(&inData[i],1);
	}
}
